package net.mcreator.biomeoverheat.network;
...
import net.mcreator.biomeoverheat.BiomeOverheatMod;

import java.util.function.Supplier;

@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD)
public class BiomeOverheatModVariables {
	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		BiomeOverheatMod.addNetworkMessage(PlayerVariablesSyncMessage.class, PlayerVariablesSyncMessage::buffer, PlayerVariablesSyncMessage::new, PlayerVariablesSyncMessage::handleData);
	}
	...
	public static class EventBusVariableHandlers {
		...
		public static void onPlayerLoggedInSyncPlayerVariables(PlayerEvent.PlayerLoggedInEvent event) {
			if (event.getEntity() instanceof ServerPlayer player)
				player.getCapability(PLAYER_VARIABLES).ifPresent(capability -> BiomeOverheatMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> player), new PlayerVariablesSyncMessage(capability)));
		}
			...
			if (event.phase == TickEvent.Phase.END && event.player instanceof ServerPlayer player) {
				player.getCapability(PLAYER_VARIABLES).ifPresent(capability -> {
					if (capability._syncDirty) {
						BiomeOverheatMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> player), new PlayerVariablesSyncMessage(capability));
		...
		public static void clonePlayer(PlayerEvent.Clone event) {
			...
			event.getOriginal().getCapability(PLAYER_VARIABLES).ifPresent(original -> {
				event.getEntity().getCapability(PLAYER_VARIABLES).ifPresent(clone -> {
					if (!event.isWasDeath()) {
						clone.Temperatur = original.Temperatur;
					}
				});
			});
		}
	...
	private static class PlayerVariablesProvider implements ICapabilitySerializable<CompoundTag> {
		...
		public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
			if (event.getObject() instanceof Player && !(event.getObject() instanceof FakePlayer))
				event.addCapability(new ResourceLocation("biome_overheat", "player_variables"), new PlayerVariablesProvider());
		}
	...
	public static class PlayerVariables implements INBTSerializable<CompoundTag> {
		...
		public CompoundTag serializeNBT() {
			...
			nbt.putDouble("Temperatur", Temperatur);
			return nbt;
		}

		@Override
		public void deserializeNBT(CompoundTag nbt) {
			Temperatur = nbt.getDouble("Temperatur");
		}
			...
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer() && message.data != null)
					Minecraft.getInstance().player.getCapability(PLAYER_VARIABLES).ifPresent(cap -> {
						cap.Temperatur = message.data().Temperatur;
					});
			});
