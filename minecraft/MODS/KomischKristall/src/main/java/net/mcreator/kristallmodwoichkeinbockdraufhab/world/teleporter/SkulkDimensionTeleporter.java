...
@EventBusSubscriber
public class SkulkDimensionTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(KristallmodwoichkeinbockdraufhabModBlocks.SKULK_DIMENSION_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(Identifier.parse("kristallmodwoichkeinbockdraufhab:skulk_dimension_portal"), poiType);
	...
	public SkulkDimensionTeleporter(ServerLevel level) {
		this.level = level;
	}
			...
			for (int box = -1; box < 2; box++) {
				for (int width = 0; width < 2; width++) {
					for (int height = -1; height < 3; height++) {
						BlockState blockState = height < 0 ? Blocks.REINFORCED_DEEPSLATE.defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int width = -1; width < 3; width++) {
			for (int height = -1; height < 4; height++) {
				if (width == -1 || width == 2 || height == -1 || height == 3) {
					...
					this.level.setBlock(mutable, Blocks.REINFORCED_DEEPSLATE.defaultBlockState(), 3);
				}
			}
		}
		BlockState portalBlockState = KristallmodwoichkeinbockdraufhabModBlocks.SKULK_DIMENSION_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, portalAxis);
		for (int width = 0; width < 2; width++) {
